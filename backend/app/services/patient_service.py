...
async def get_patients(
    ...
    try:
        ...
        if owner_id is not None:
            filters.append(Patient.user_id == owner_id)
        ...
        logger.info(
            f"Retrieved {len(patients)} patients from clinic {clinic_id}, page {pagination.page}"
        )
...
async def create_patient(
    ...
    try:
        ...
        patient_data = patient_in.model_dump(exclude={"user_id"})
        patient = Patient(
...
async def get_patient_by_id(
    ...
    try:
        ...
        logger.error(
            f"Invalid UUID format - clinic_id: {clinic_id}, patient_id: {patient_id}"
        )
        raise
    except Exception as e:
...
async def update_patient(
    ...
    try:
        ...
        patient = await repo.get_by_patient_id(
            uuid.UUID(patient_id), clinic_id=uuid.UUID(clinic_id)
        )
        if patient is not None and owner_id is not None and patient.user_id != owner_id:
            patient = None
        if not patient:
        ...
        update_data = patient_in.model_dump(exclude_unset=True, exclude={"user_id"})
        if not update_data:
            return patient
...
async def delete_patient(
    db: AsyncSession, clinic_id: str, patient_id: str, user_id: Optional[str] = None
) -> None:
    from app.repositories.patient import PatientRepository
...
async def search_patients(
    ...
    try:
        ...
        if search.isdigit() and len(search) == 10:
            patients = await repo.search_by_phone(
                search, clinic_id=c_id, owner_id=owner_id, offset=offset, limit=limit
            )
        elif "@" in search:
            patients = await repo.search_by_email(
                search, clinic_id=c_id, owner_id=owner_id, offset=offset, limit=limit
            )
        else:
            patients = await repo.search_by_name(
                search, clinic_id=c_id, owner_id=owner_id, offset=offset, limit=limit
            )
