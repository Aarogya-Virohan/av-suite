...
async def upload_patient_document(
    ...
    if scope == CapabilityScope.OWN and not await service.patient_belongs_to_therapist(
        clinic.id, patient_id, user.id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only upload documents for your own patients.",
        )
...
async def create_document(
    ...
    if scope == CapabilityScope.OWN and not await service.patient_belongs_to_therapist(
        clinic.id, payload.patient_id, user.id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only upload documents for your own patients.",
...
@router.get(
    "/patients/{patient_id}/documents", response_model=PatientDocumentListResponse
)
async def list_patient_documents_by_patient(
    ...
    if scope == CapabilityScope.OWN:
        documents = await service.list_documents_for_therapist(
            clinic.id,
            user.id,
...
async def get_document(
    ...
    try:
        ...
        if (
            ...
            and not await service.document_belongs_to_therapist(clinic.id, id, user.id)
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only access documents for your own patients.",
            )
        return PatientDocumentResponse.model_validate(document)
...
async def update_document(
    ...
    try:
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only update documents for your own patients.",
            )
...
async def delete_document(
    ...
    try:
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only delete documents for your own patients.",
            )
