...
@pytest.mark.asyncio
async def test_public_booking_request_and_approval_flow(
    client: AsyncClient, auth_headers: dict
):
    # 1. Staff View Pending Booking Requests
    list_req_res = await client.get(
        f"{settings.API_V1_PREFIX}/appointment-requests", headers=auth_headers
    )
    ...
    # 3. Public Booking Request (Unauthenticated with clinic slug)
    booking_req_payload = {
        "name": "Vikas Verma",
        "phone": "9899887766",
        "age": 32,
    ...
    assert (
        approve_data.get("appointment_id") is not None
        or approve_data.get("request", {}).get("status") == "approved"
    )
